 ...
 * Extracted from she.ships by Crafter Station.
 */
	...
	for (let step = 0; step < substeps; step++) {
		...
		for (let i = 0; i < count; i++) {
			...
			if (CR > 0) {
				let cdx = 0;
				let cdy = 0;
				if (px[i] < cx0 && py[i] < cy0) {
					cdx = px[i] - cx0;
					cdy = py[i] - cy0;
				} else if (px[i] > cx1 && py[i] < cy0) {
					cdx = px[i] - cx1;
					cdy = py[i] - cy0;
				} else if (px[i] < cx0 && py[i] > cy1) {
					cdx = px[i] - cx0;
					cdy = py[i] - cy1;
				} else if (px[i] > cx1 && py[i] > cy1) {
					cdx = px[i] - cx1;
					cdy = py[i] - cy1;
				}
				if (cdx !== 0 || cdy !== 0) {
		...
		for (let pass = 0; pass < 3; pass++) {
			gridCnt.fill(0);
			for (let i = 0; i < count; i++) {
				const gcx = Math.max(0, Math.min(gridW - 1, ((px[i] - ox) / cellSize) | 0));
				const gcy = Math.max(0, Math.min(gridH - 1, ((py[i] - oy) / cellSize) | 0));
				const ci = gcy * gridW + gcx;
			...
			for (let gy2 = yStart; gy2 !== yEnd; gy2 += yStep) {
				for (let gx2 = xStart; gx2 !== xEnd; gx2 += xStep) {
					const ci = gy2 * gridW + gx2;
					...
					for (let a = 0; a < cnt; a++) {
						...
						for (let ndy = 0; ndy <= 1; ndy++) {
							const ny2 = gy2 + ndy;
							if (ny2 < 0 || ny2 >= gridH) continue;
							const dxStart = ndy === 0 ? 1 : -1;
							for (let ndx = dxStart; ndx <= 1; ndx++) {
								const nx2 = gx2 + ndx;
